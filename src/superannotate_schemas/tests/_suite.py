...
from superannotate_schemas.compat import PY3
from superannotate_schemas.validators import validators
...
def _find_suite():
    ...
    root = FilePath(superannotate_schemas.__file__).parent().sibling("json")
...
class Suite(object):
    ...
    def _remotes(self):
        superannotate_schemas_suite = self._root.descendant(["bin", "superannotate_schemas_suite"])
        remotes = subprocess.check_output(
            [sys.executable, superannotate_schemas_suite.path, "remotes"],
        )
        return {
...
class _Test(object):
    ...
    def to_unittest_method(self, skip=lambda test: None, **kwargs):
        if self.valid:
            ...
            def fn(this):
                with this.assertRaises(superannotate_schemas.ValidationError):
                    self.validate(**kwargs)
    ...
    def validate(self, Validator, **kwargs):
        resolver = superannotate_schemas.RefResolver.from_schema(
            schema=self.schema,
            store=self._remotes,
            id_of=Validator.ID_OF,
        )
        superannotate_schemas.validate(
            instance=self.data,
            schema=self.schema,
            cls=Validator,
            resolver=resolver,
    ...
    def validate_ignoring_errors(self, Validator):  # pragma: no cover
        ...
        except superannotate_schemas.ValidationError:
            pass
